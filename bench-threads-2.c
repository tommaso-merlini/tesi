...
static void fill_real(bench_real *x, size_t n)
{
	size_t i;
	for (i = 0; i < n; ++i) x[i] = (double) (i % 97);
}

static void fill_complex(BENCH_FFTW(complex) *x, size_t n)
{
	size_t i;
	for (i = 0; i < n; ++i) {
		x[i][0] = (double) (i % 97);
...
static int bench_c2c(int rank, int n, int iters)
{
	...
	double t0, t1;
	BENCH_FFTW(complex) *in;
	BENCH_FFTW(complex) *out;
	BENCH_FFTW(plan) p;

	if (rank == 2) total *= (size_t) n;
	if (rank == 3) total *= (size_t) n * (size_t) n;

	in = BENCH_FFTW(malloc)(sizeof(BENCH_FFTW(complex)) * total);
	out = BENCH_FFTW(malloc)(sizeof(BENCH_FFTW(complex)) * total);
	if (!in || !out) {
...
static int bench_r2c(int rank, int n, int iters)
{
	...
	in = BENCH_FFTW(malloc)(sizeof(bench_real) * in_total);
	out = BENCH_FFTW(malloc)(sizeof(BENCH_FFTW(complex)) * out_total);
	if (!in || !out) {
...
static int bench_c2r(int rank, int n, int iters)
{
	...
	in = BENCH_FFTW(malloc)(sizeof(BENCH_FFTW(complex)) * in_total);
	out = BENCH_FFTW(malloc)(sizeof(bench_real) * out_total);
	if (!in || !out) {
...
static int bench_r2r(int rank, int n, int iters)
{
	...
	in = BENCH_FFTW(malloc)(sizeof(bench_real) * total);
	out = BENCH_FFTW(malloc)(sizeof(bench_real) * total);
	if (!in || !out) {
